...
// QuerySubmenus chains the current query on the submenus edge.
func (mq *MenuQuery) QuerySubmenus() *MenuQuery {
	query := &MenuQuery{config: mq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
...
//  WithSubmenus tells the query-builder to eager-loads the nodes that are connected to
// the "submenus" edge. The optional arguments used to configure the query builder of the edge.
func (mq *MenuQuery) WithSubmenus(opts ...func(*MenuQuery)) *MenuQuery {
	query := &MenuQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
	mq.withSubmenus = query
	return mq
}
...
func (mq *MenuQuery) sqlAll(ctx context.Context) ([]*Menu, error) {
	...
	if query := mq.withParent; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].menu_submenus; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "menu_submenus" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := mq.withSubmenus; query != nil {
		...
		query.Where(predicate.Menu(func(s *sql.Selector) {
			s.Where(sql.InValues(menu.SubmenusColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.menu_submenus
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "menu_submenus" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "menu_submenus" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Submenus = append(node.Edges.Submenus, n)
		}
	}
