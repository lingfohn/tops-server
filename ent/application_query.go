...
func (aq *ApplicationQuery) QueryNamespace() *NamespaceQuery {
...
func (aq *ApplicationQuery) QueryInstances() *InstanceQuery {
...
func (aq *ApplicationQuery) QueryConfig() *HelmConfigQuery {
	query := &HelmConfigQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
//  WithNamespace tells the query-builder to eager-loads the nodes that are connected to
// the "namespace" edge. The optional arguments used to configure the query builder of the edge.
func (aq *ApplicationQuery) WithNamespace(opts ...func(*NamespaceQuery)) *ApplicationQuery {
	query := &NamespaceQuery{config: aq.config}
...
func (aq *ApplicationQuery) WithInstances(opts ...func(*InstanceQuery)) *ApplicationQuery {
	query := &InstanceQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *ApplicationQuery) WithConfig(opts ...func(*HelmConfigQuery)) *ApplicationQuery {
	query := &HelmConfigQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *ApplicationQuery) sqlAll(ctx context.Context) ([]*Application, error) {
	...
	if aq.withNamespace != nil || aq.withProject != nil || aq.withConfig != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := aq.withNamespace; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].namespace_applications; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "namespace_applications" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := aq.withProject; query != nil {
		...
		nodeids := make(map[int][]*Application)
		for i := range nodes {
			if fk := nodes[i].project_applications; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "project_applications" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := aq.withInstances; query != nil {
		...
		for _, n := range neighbors {
			fk := n.application_instances
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "application_instances" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "application_instances" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Instances = append(node.Edges.Instances, n)
		}
	}

	if query := aq.withConfig; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Application)
		for i := range nodes {
			if fk := nodes[i].application_config; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "application_config" returned %v`, n.ID)
			}
			for i := range nodes {
